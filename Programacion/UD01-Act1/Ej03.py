...
# El caracter cifrado de la ‘a’ es ‘z’, el de la ‘b’ es ‘y’, el de la ‘c’ es ‘x’ y asi sucesivamente.
...
if operacion == "C":             # cifrar el mensaje
    ...
    while pos_letra_mensaje < tamaño_mensaje:
        if mensaje[pos_letra_mensaje] in ALFABETO:
            ...
            mje_cifrado += ALFABETO_INV[posicion]
        else:
    ...
    while pos_letra_mensaje < tamaño_mensaje:
        if mensaje[pos_letra_mensaje] in ALFABETO:
            posicion = ALFABETO_INV.find(mensaje[pos_letra_mensaje])
            mje_claro += ALFABETO[posicion]
        else:
